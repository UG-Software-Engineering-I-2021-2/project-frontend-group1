...
const RubricCard = (props: { data: Rubric, course: string,courseCode: string }) => {
  ...
  const clickCreateRubric = () => {
    if(props.data.canGrade || props.data.state === "Disponible para calificar" || props.data.state === "Cumplidos") {
      history.push(`/grade-rubric?code=${props.data.code}&course=${props.course}&courseCode=${props.courseCode}&canGrade=${props.data.canGrade}`);
      return
    }
      ...
      >
        ...
        <Box mt={2}>
          <Box color={useColorModeValue("gray.700", "white")}>
            <Heading size="lg">Título: {props.data.title || "No defined"}</Heading>
            <Heading size="lg">Evaluación: {props.data.evaluation}</Heading>
